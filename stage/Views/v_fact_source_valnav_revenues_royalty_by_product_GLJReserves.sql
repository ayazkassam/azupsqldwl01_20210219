CREATE VIEW [stage].[v_fact_source_valnav_revenues_royalty_by_product_GLJReserves]
AS SELECT entity_id entity_key,
	...
	FROM (
		SELECT entity_id,
			...
			scenario_type,
			normalized_time_key
		from [stage].v_fact_source_valnav_royalty_by_product_calculations_GLJReserves
		, (	SELECT 'GRS' gross_net UNION ALL 
			SELECT 'WI' UNION ALL 
			SELECT 'RI') gn
	) rev_src
	WHERE cad <> 0 
	...
	FROM (
		SELECT entity_id,
			...
			cad
		from [stage].v_fact_source_valnav_royalty_by_product_calculations_GLJReserves
		UNPIVOT (cad FOR royalty_type IN (crown_royalty, gor_royalty, freehold_royalty, indian_royalty
										, mineral_tax_royalty, sask_cap_surcharge_royalty)) as roy
	) roy_src
	WHERE cad <> 0;