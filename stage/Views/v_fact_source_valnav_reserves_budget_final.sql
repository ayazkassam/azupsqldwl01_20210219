CREATE VIEW [stage].[v_fact_source_valnav_reserves_budget_final]
...
FROM [stage].v_fact_source_valnav_reserves_budget
--
UNION ALL
--
SELECT entity_guid entity_key,
...
SELECT entity_id entity_key,
	  ...
	  '-1' normalized_time_key,
	  scenario_type,
	  ri_volume_imp as imperial_volume,
	  ri_volume_boe as boe_volume,
	  ri_volume_met as metric_volume,
	  ri_volume_mcfe as mcfe_volume
FROM [stage].v_fact_source_valnav_reserves_budget_ri
)rs
GROUP BY entity_key,
	   activity_date_key,
	   account_key,
