CREATE VIEW [stage].[v_prodview_injection_volumes_incr] AS SELECT keymigrationsource
	, compida
	, cc_num
	, completionname
	, typmigrationsource
...
from (
	...
	join stage_prodview.t_pvt_pvunitallocmonthday_incr pc on pu.idflownet = pc.idflownet and puc.idrec = pc.idreccomp
	left outer join stage_prodview.t_pvt_pvflownetheader fh on pu.idflownet = fh.idflownet
	--left outer join t_stg_prodview_pressures_facts_incr pucp ON puc.idflownet = pucp.idflownet AND puc.idrec = pucp.idrecparent AND pc.dttm = pucp.full_date
	LEFT OUTER JOIN stage_prodview.t_pvt_pvunitcompparam_incr comp
      ON pc.idrecparam = comp.idrec
       AND puc.idrec = comp.idrecparent
	where pu.typ2 IN ('Water Injection', 'Well (inj)', 'Well (source)')
	AND cast (pc.dttm as date) >= cast (fh.syslockdatemaster - 35 as date)
