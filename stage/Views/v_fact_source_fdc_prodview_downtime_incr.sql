CREATE VIEW [stage].[v_fact_source_fdc_prodview_downtime_incr]
...
as (
	SELECT site_id
		...
		--
		, (isnull(vollostgas,0) * isnull(cf.gas_boe_thermal,1)) 
			+ (isnull(oil_lost_volume,0) + (isnull(condensate_lost_volume,0) + isnull(ethane_lost_volume,0) + isnull(propane_lost_volume,0) 
			+ isnull(butane_lost_volume,0) + isnull(pentane_lost_volume,0)) * isnull(cf.liquid_si_to_imp_conv_factor,1)) total_boe_volume
	...
	FROM (
		SELECT CASE WHEN UPPER (dtm.typmigrationsource) LIKE '%PVR%' THEN pvr.company_id + '.' + pvr.battery_id + '.' + pvr.well_id 
			...
			, dtm.cc_num
			, dtm.codedowntm1
			, CASE WHEN dtm.codedowntm1 IS NULL THEN '-1' ELSE dtm.codedowntm1 END scenario_key, dtm.vollostgas
		...
		, (	SELECT *
			FROM [stage].[t_prodview_downtime_hours_volumes_incr] p
