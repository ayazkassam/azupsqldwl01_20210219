CREATE VIEW [stage].[v_valnav_uwi_working_interest]
AS SELECT wi.uwi,
	wi.entity_object_id,
...
FROM (
	...
	FROM (
		...
		FROM stage_valnav.t_budget_ENT_REVERSION_INTEREST eri
	...
	LEFT OUTER JOIN (
				...
				LEFT OUTER JOIN (
							...
							, (	
								...
								FROM stage_valnav.t_budget_ENT_SHARED_PLAN_DATA pd
								left outer join (
											select ed.PARENT_ID
												, ed.PRODUCT_LIST_ID
												, p.PRODUCT_ID
											from stage_valnav.t_budget_ENT_DETAIL ed
											left outer join stage_valnav.t_budget_FISC_PRODUCT_LIST_VALUE p on ed.PRODUCT_LIST_ID = p.PARENT_ID
											where IS_PRIMARY = 1
...
/*LEFT OUTER JOIN (
		...
		FROM (
				SELECT *
				FROM stage_valnav.t_budget_dbo.RESULTS_COST_DETAIL rcd
		...
		LEFT OUTER JOIN stage_valnav.t_budget_dbo.RESULTS_LOOKUP rl ON rcd.result_id = rl.result_id
		LEFT OUTER JOIN	stage_valnav.t_budget_dbo.ENTITY e ON rl.entity_id = e.object_id
		INNER JOIN (
				SELECT variable_value
				FROM stage.dbo.T_CTRL_VALNAV_ETL_VARIABLES
				WHERE variable_name = 'RESERVE_CATEGORY_ID'
		) vev ON rl.reserve_category_id = vev.variable_value
		INNER JOIN (
				select OBJECT_ID, NAME
				from BNPBUDGETSCOPY.dbo.FISC_CAP_COST
		) cl ON rcd.COST_DEFINITION_ID = cl.OBJECT_ID
		INNER JOIN (
				SELECT fsp.plan_id, fs.*
				FROM stage_valnav.t_budget_dbo.FISC_SCENARIO fs
				LEFT OUTER JOIN stage_valnav.t_budget_dbo.FISC_SCENARIO_PARAMS fsp ON (fs.object_id = fsp.parent_id)
