CREATE VIEW [stage].[v_fact_source_valnav_financial_GLJReserves]
AS SELECT entity_key,
	...
	FROM (
			...
			FROM [stage].v_fact_source_valnav_capital_GLJReserves
	...
	SELECT entity_key,
		...
		gross_net,
		normalized_time_key,
		'GLJ Reserves' scenario_type,
		cad,
		k_cad
	FROM [stage].[v_fact_source_valnav_economics_GLJReserves_final]
	...
	SELECT entity_key,
		   ...
		   cad,
		   k_cad
	FROM [stage].[v_fact_source_valnav_royalties_GLJReserves]
	...
	SELECT entity_key,
		   ...
		   cad,
		   k_cad
	FROM [stage].[v_fact_source_valnav_revenues_royalty_by_product_GLJReserves];