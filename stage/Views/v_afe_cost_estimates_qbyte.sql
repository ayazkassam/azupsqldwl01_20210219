CREATE VIEW [stage].[v_afe_cost_estimates_qbyte]
AS SELECT
                 afe.afe_num,
                 /*
                 CASE
                 ...
                 */
                 afe.afe_name,
                 SUBSTRING (LTRIM (RTRIM (es.cc_num)), 1, 100) AS cc_num,
                 SUBSTRING (LTRIM (RTRIM (cc.uwi)), 1, 100) AS uwi,
                 SUBSTRING (LTRIM (RTRIM (es.gl_account)), 1, 10) AS gl_account,
                 SUBSTRING (LTRIM (RTRIM (es.gl_net_account)), 1, 10) AS gl_net_account,
                 es.grs_net_indicator,
                 SUBSTRING (LTRIM (RTRIM (es.scenario)), 1, 10) AS scenario,
                 ...
                 'UNKNOWN_VENDOR' AS vendor_id, --'-1' AS vendor_id,
                 ...
                 CASE
                    ...
                    ELSE
                       DATEADD (MONTH, DATEDIFF (MONTH, 0, afe.afe_date) + 1, 0) - 1
                 END AS afe_date,
                 CASE
                    WHEN afe.acct_yr < am.min_yr
                    ...
                    WHEN afe.acct_yr IS NULL
                       THEN 'A1980_01'
                    ELSE
                         'A' + CONVERT (NVARCHAR (4), DATEPART (YEAR, afe.afe_date))
                       + '_' + SUBSTRING (CONVERT (NVARCHAR (6), afe.afe_date, 112), 5, 2)
                 END AS acct_mnth,
                 afe.acct_yr,
                 CASE
                    WHEN afe.prod_yr BETWEEN pm.min_yr AND pm.max_yr
                       THEN    'P' + CONVERT (NVARCHAR (4), DATEPART (YEAR, afe.afe_date))
                             + '_' + SUBSTRING (CONVERT (NVARCHAR (6), afe.afe_date, 112), 5, 2)
            ...
            FROM
                 (
                     SELECT
                            est.afe_num,
                            CASE
                               WHEN est.cc_num IS NOT NULL
                                  THEN REPLACE (est.cc_num, '***DUMMY**', 'DUMMY CC')
                               ELSE
                                  'CC_NOT_REQUIRED'
                            END AS cc_num,
                            est.major_acct + '_' + est.minor_acct AS gl_account,
                            CASE
                               WHEN maj.net_major_acct IS NOT NULL
                                  THEN maj.net_major_acct + '_' + est.minor_acct
                               ELSE
                                  est.major_acct + '_' + est.minor_acct
                            END AS gl_net_account,
                            CASE maj.gross_or_net_code
                               WHEN 'N'
                                  THEN 'NET'
                               ELSE
                                  'GRS'
                            END AS major_grs_net_flag,
                            maj.class_code AS major_class_code,
                            est.grs_net_indicator,
                            est.scenario,
                            est.estimate_amount
                       FROM
                            (
                             /* Using a single pass against the [dbo].[v_qbyte_source_cost_estimates] data source,
                                obtain original and supplement cost estimates for both gross and net transactions
                             */
                                 SELECT
                                         cc_num,
                                         afe_num,
                                         major_acct,
                                         minor_acct,
                                         CASE WHEN cc in ('grs_supp','orig_gross_est') then 'GRS' 
											 ELSE 'NET'
										 END grs_net_indicator,
										  CASE WHEN cc in ('grs_supp','net_supp') then 'Supplement' 
											 ELSE 'Original'
										 END scenario,
										 cast(NULLIF(estimate_amount,'NULL') as numeric(16,2)) estimate_amount
									...
									(
										...
										SELECT 
												...
												major_acct,
												minor_acct,
												ISNULL(cast(orig_gross_est as varchar(20)),'NULL') orig_gross_est,
												ISNULL(cast(orig_net_est as varchar(20)),'NULL') orig_net_est,
												cast(gross_est as varchar(20)) gross_est,
												ISNULL(cast(ISNULL (supp_gross_est, gross_est) as varchar(20)),'NULL') grs_supp,
												ISNULL(cast(ISNULL (supp_net_est, net_est) as varchar(20)),'NULL') net_supp
	
										from
												[stage].[t_source_cost_estimates_qbyte] 
										
									) pvt
									UNPIVOT
									(
										estimate_amount for cc in (orig_gross_est, 
																	orig_net_est,
																	grs_supp, 
																	net_supp																	
																 )
                             ...
                             /*
                                   WHERE
                                          estimate_amount IS NOT NULL
                             */
                            ) est
                 INNER JOIN
                            [stage].[t_qbyte_major_accounts] maj
                         ON
                            (est.major_acct = maj.major_acct)
                      /* WHERE est.minor_acct <> 'CLR' */
                ) es
LEFT OUTER JOIN
                (
                 /* Retrieve a listing of valid AFEs from QByteFM, along with pertinent attribute information */
                           SELECT
                                  ...
                                  CAST(NULL as varchar(20)) AS project_approval_status,
                                  CAST(NULL as varchar(20)) AS project_execution_status,
                                  ISNULL (DATEADD (dd, DATEDIFF (dd, 0, ae.afe_start_date), 0), DATEADD (dd, DATEDIFF (dd, 0, ae.create_date), 0)) AS afe_date,
...
LEFT OUTER JOIN
                (
                 /* Retrieve a listing of valid cost centres from QByteFM, along with pertinent attribute information */
                          SELECT
                                 c.cc_num,
                                 --
                                 CASE
             ...
             ON
                (es.cc_num = cc.cc_num) /* Used to derive cost centre capital or dry hole expense flag */
     INNER JOIN
                (SELECT
