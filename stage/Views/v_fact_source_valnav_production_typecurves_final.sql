CREATE VIEW [stage].[v_fact_source_valnav_production_typecurves_final]
...
SELECT				entity_guid entity_key,
                    ...
                    END  AS mcfe_volume
               FROM (select *
			         from [stage].v_fact_source_valnav_production_typecurves
...
(
 SELECT             vp.entity_guid entity_key,
               ...
               FROM (SELECT 'GRS' AS grs_net
			        ...
			        [stage].v_fact_source_valnav_production_typecurves  vp
			        LEFT OUTER JOIN
                    (SELECT *
                       FROM [data_mart].t_dim_entity
                      WHERE is_valnav=1
					  AND UPPER (LTRIM(RTRIM (capital_type))) = 'BASE') cb
					ON vp.entity_guid = cb.entity_key                   
...
SELECT DISTINCT entity_guid,
                           ...
                           scenario_type,
                           MAX (boe_volume)
                              OVER (PARTITION BY entity_guid,
                                                 reserve_category_id,
                                                 grs_net,
                                                 scenario,
                                                 scenario_type)
                              AS max_boe_volume
             FROM (  SELECT entity_guid,
                            reserve_category_id,
                            step_date,
                            onstream_date,
                            gn.grs_net,
                       ...
                       FROM [stage].v_fact_source_valnav_production_typecurves f,
                            (SELECT 'GRS' AS grs_net
                             UNION ALL
                             SELECT 'WI' AS grs_net
                             UNION ALL
