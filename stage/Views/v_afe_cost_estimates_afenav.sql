CREATE VIEW [stage].[v_afe_cost_estimates_afenav]
AS SELECT
                 afe.afe_num,
                 /*
                 CASE
                 ...
                 */
                 ...
                 cc.uwi,
                 es.gl_account,
                 es.gl_account AS gl_net_account,
                 es.scenario,
                  ...
                  SELECT
                    ...
                    FROM
                         (
                          --
                              SELECT
                                     afenumber AS afe_num,
                                     amount_date AS time_period,
                                     DATEPART (YEAR, amount_date) AS acct_yr,
                                     net_grs AS grs_net_flag,
                                     CASE
                                        WHEN [major_acct] IS NOT NULL AND [minor_acct] IS NOT NULL
                                           THEN [major_acct] + '_' + [minor_acct]
                                     END AS gl_account,
                                     ctrl.class_code AS maj_class_code,
                                     'Incurred' AS scenario, /* AFENav Estimates */
                                     amount
                                FROM
                                     [stage].[t_source_cost_estimates_afenav] est
                          INNER JOIN
                                     (
                                               SELECT
                                                      x.[source_minor_acct] AS afe_type,
                                                      x.[target_minor_acct] AS minor_acct,
                                                      x.[target_major_acct] AS major_acct,
                                                      maj.net_major_acct,
                                                      maj.class_code
                                                 FROM
                                                      [stage].[t_ctrl_accounts_xref] x
                                      LEFT OUTER JOIN
                                                      [stage].[t_qbyte_major_accounts] maj
                                                   ON
                                                      (x.[target_major_acct] = maj.major_acct)
                                                WHERE
                                                      x.[source_app]        = 'AFENavigator'
                                                  AND x.[source_major_acct] = 'AFE_TYPE'
                                     ) ctrl
                                  ON
                                     (est.afe_type = ctrl.afe_type)
                          --
                         ) afenav
                ) es
LEFT OUTER JOIN
