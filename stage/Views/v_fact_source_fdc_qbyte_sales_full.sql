CREATE VIEW [stage].[v_fact_source_fdc_qbyte_sales_full]
...
SELECT entity_key,
	   ...
	   ---
	   /*
	   CASE WHEN isnull(ngl_grs_metric_volume,0) <> isnull(ngl_tgrs_metric_volume,0)
		    THEN ngl_tgrs_metric_volume
			ELSE ngl_grs_metric_volume
	   END total_ngl_metric_volume,
	   CASE WHEN isnull(ngl_grs_imperial_volume,0) <> isnull(ngl_tgrs_imperial_volume,0)
		    THEN ngl_tgrs_imperial_volume
			ELSE ngl_grs_imperial_volume
	   END total_ngl_imperial_volume,
	   CASE WHEN isnull(ngl_grs_boe_volume,0) <> isnull(ngl_tgrs_boe_volume,0)
		    THEN ngl_tgrs_boe_volume
			ELSE ngl_grs_boe_volume
	   END total_ngl_boe_volume,
	   CASE WHEN isnull(ngl_grs_mcfe_volume,0) <> isnull(ngl_tgrs_mcfe_volume,0)
		    THEN ngl_tgrs_mcfe_volume
			ELSE ngl_grs_mcfe_volume
	   END total_ngl_mcfe_volume
	   --
	   */
FROM [stage].t_stg_qbyte_volumes_facts
--
UNION ALL
--
SELECT entity_key,
