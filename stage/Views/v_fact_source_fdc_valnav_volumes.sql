CREATE VIEW [stage].[v_fact_source_fdc_valnav_volumes]
...
FROM (
	...
	FROM (
		...
		SELECT uwi entity_key,
			...
			cube_child_member scenario_key,
		...
		FROM (
			...
			SELECT e.uwi, sc.cube_child_member, vp.*
			...
			INNER JOIN (
					...
					AND cube_child_member like 'Working%' /* This filter ensures ONLY Working Budget is selected*/
			...
			/*-- OTHER SCENARIOS (excluding Working and Current Approved Budget)  --> Current Budget Year Varable + 1*/
			SELECT e.uwi, sc.cube_child_member, vp.*
			FROM (	SELECT CAST(variable_value +'0101' AS INT) start_date,
			...
			INNER JOIN (
					...
					WHERE variable_name = 'VOLUMES_DATA_SCENARIO'
					AND variable_value IS NOT NULL
					AND variable_value NOT LIKE (	SELECT DISTINCT '%'+variable_value+'%'
													FROM [stage].[t_ctrl_valnav_etl_variables]
													WHERE variable_name = 'VALNAV_CURRENT_APPROVED_BUDGET') /* exclue Valnav Current Approved Budget -- part of daily etl*/
					AND cube_child_member NOT like 'Working%' /* Working budget is above so all other scenarios except Working*/
			) sc ON vp.scenario_key = sc.variable_value
			INNER JOIN (
					SELECT *
					FROM [stage].[t_ctrl_valnav_etl_variables]
