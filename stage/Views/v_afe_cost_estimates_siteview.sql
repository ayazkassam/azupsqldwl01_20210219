CREATE VIEW [stage].[v_afe_cost_estimates_siteview]
AS SELECT
                 afe.afe_num,
                 /*
                 CASE
                 ...
                 */
                 ...
                 END AS budget_year,
                 ISNULL (CAST(afe.project_approval_status AS NVARCHAR(50)), 'UNKNOWN_APPROVAL_STATUS') AS project_approval_status,
                 ISNULL (CAST(afe.project_execution_status AS NVARCHAR(50)), 'UNKNOWN_EXECUTION_STATUS') AS project_execution_status,
                 CASE
            ...
            FROM
                 (
                  /* Obtain all Siteview AFE estimates irregardless of what net major minor account is tied to each
                     transaction
                  */
                  SELECT
                         sv.*
                    FROM
                          ...
                          */
                          SELECT
                                 ...
                                 amount
                            FROM
                                 [stage].[t_source_cost_estimates_siteview]
                           WHERE
                                 is_valid_afe = 'Y'
						   AND  scenario = 'SITEVIEW'
                          --
                          UNION ALL
                          --
                          SELECT
                ...
                (
                 /* Retrieve a listing of valid AFEs from QByteFM, along with pertinent attribute information */
                           SELECT
                                  ...
                                  NULL AS project_approval_status,
                                  NULL AS project_execution_status,
     ...
     INNER JOIN
                (SELECT
                   ...
                   FROM
                        [stage].[t_ctrl_etl_variables]
                  WHERE
                        variable_name IN ('CAPITAL_ACTIVITY_DATE_START_YEAR', 'CAPITAL_ACTIVITY_DATE_END_YEAR')
                ) pm
             ON
