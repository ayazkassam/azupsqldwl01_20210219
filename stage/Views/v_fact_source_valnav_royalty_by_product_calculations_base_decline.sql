CREATE VIEW [stage].[v_fact_source_valnav_royalty_by_product_calculations_base_decline]
AS SELECT e.object_id entity_id,
	...
	cast(vev.cube_child_member as varchar(50)) AS reserve_category_id,
...
inner join (
		...
		from (
				...
				FROM [stage].dbo.t_stg_valnav_budget_ent_custom_field_def c
				join t_ctrl_valnav_etl_variables v on ltrim(rtrim(c.string_value)) = v.variable_value
...
LEFT OUTER JOIN (
		...
		FROM [stage].dbo.t_valnav_onstream_date_budget
) od ON rl.entity_id = od.object_id
*/

LEFT OUTER JOIN stage_valnav.t_basedecline_results_btax rb ON rb.result_id = rl.result_id
