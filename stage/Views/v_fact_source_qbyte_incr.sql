CREATE VIEW [stage].[v_fact_source_qbyte_incr]
AS SELECT qb_li_vc.li_id,
	...
	CASE WHEN qb_li_vc.li_vol IS NULL OR UPPER (acct.account_level_02) = 'OPERATING EXPENSES' THEN NULL 
		ELSE qb_li_vc.li_vol * ISNULL (acct.si_to_imp_conv_factor, 1) 
			* (CASE WHEN acct.product_code = 'GAS' THEN ISNULL (acct.mcfe6_thermal, 1) ELSE ISNULL (acct.mcfe6_thermal, 6) END)
		END AS qb_mcfe_volume,
	(CASE WHEN qb_li_vc.li_vol IS NULL OR UPPER (acct.account_level_02) = 'OPERATING EXPENSES' THEN NULL 
		ELSE qb_li_vc.li_vol * ISNULL (acct.si_to_imp_conv_factor, 1) 
			* (CASE WHEN acct.product_code = 'GAS' THEN ISNULL (acct.mcfe6_thermal, 1) ELSE ISNULL (acct.mcfe6_thermal, 6) END) 
	...
	(CASE WHEN fv.fixed_opex_percent IS NULL THEN NULL 
		ELSE (CASE	WHEN org.op_curr_code = 'CAD' AND qb_li_vc.li_amt IS NOT NULL THEN qb_li_vc.li_amt
					WHEN org.reporting_curr_code = 'CAD' AND qb_li_vc.org_rep_curr_amt IS NOT NULL THEN  qb_li_vc.org_rep_curr_amt
				ELSE NULL END) * fv.fixed_opex_percent / 100
		END) * ISNULL (acct.rev_multiplier, 1) AS cad_fixed,
	(CASE WHEN fv.variable_opex_percent IS NULL THEN NULL
		ELSE (CASE	WHEN org.op_curr_code = 'CAD' AND qb_li_vc.li_amt IS NOT NULL THEN qb_li_vc.li_amt
					WHEN org.reporting_curr_code = 'CAD' AND qb_li_vc.org_rep_curr_amt IS NOT NULL THEN  qb_li_vc.org_rep_curr_amt
				ELSE NULL END) * fv.variable_opex_percent / 100
		END) * ISNULL (acct.rev_multiplier, 1) AS cad_variable,
	(CASE WHEN fv.econ_fixed_percent IS NULL THEN NULL 
		ELSE (CASE	WHEN org.op_curr_code = 'CAD' AND qb_li_vc.li_amt IS NOT NULL THEN qb_li_vc.li_amt
					WHEN org.reporting_curr_code = 'CAD' AND qb_li_vc.org_rep_curr_amt IS NOT NULL THEN  qb_li_vc.org_rep_curr_amt
				ELSE NULL END) * fv.econ_fixed_percent / 100
		END) * ISNULL (acct.rev_multiplier, 1) AS cad_econ_fixed,
	(CASE WHEN fv.econ_variable_gas_percent IS NULL THEN NULL
		ELSE (CASE	WHEN org.op_curr_code = 'CAD' AND qb_li_vc.li_amt IS NOT NULL THEN qb_li_vc.li_amt
					WHEN org.reporting_curr_code = 'CAD' AND qb_li_vc.org_rep_curr_amt IS NOT NULL THEN qb_li_vc.org_rep_curr_amt
				ELSE NULL END) * fv.econ_variable_gas_percent / 100
		END) * ISNULL (acct.rev_multiplier, 1) AS cad_econ_variable_gas,
	(CASE WHEN fv.econ_variable_oil_percent IS NULL THEN NULL 
		ELSE (CASE	WHEN org.op_curr_code = 'CAD' AND qb_li_vc.li_amt IS NOT NULL THEN qb_li_vc.li_amt
					WHEN org.reporting_curr_code = 'CAD' AND qb_li_vc.org_rep_curr_amt IS NOT NULL THEN qb_li_vc.org_rep_curr_amt
				ELSE NULL END) * fv.econ_variable_oil_percent / 100
		END) * ISNULL (acct.rev_multiplier, 1) AS cad_econ_variable_oil,                  
...
FROM (/* Obtain all line item entries corresponding to posted vouchers */
		SELECT li.li_id,
			li.cc_num,
			li.voucher_id,
			li.major_acct,
		...
		FROM (	
				SELECT l.*,
					...
					CAST (CASE maj.gross_or_net_code WHEN 'N' THEN 2 ELSE 1 /*--'GRS'*/ END AS INT) AS grs_net,
					maj.class_code
				FROM [stage].[t_qbyte_line_items] l
				LEFT OUTER JOIN [stage].[t_qbyte_major_accounts] maj ON (l.major_acct = maj.major_acct)
				WHERE l.minor_acct <> 'CLR') li
			, (	SELECT v.*
				FROM [stage].[t_qbyte_vouchers_incr] v
		...
		SELECT li.li_id,
			...
			CASE WHEN afe_op.afe_num IS NOT NULL
					...
					AND li.grs_net_flag = 'NET'
					AND ISNULL (afe_op.net_estimate_pct, 0) <> 0
				THEN li.reporting_curr_amt  * 100  / ISNULL(afe_op.net_estimate_pct, 100)	/* Gross up only posted accruals for operated AFE's */
				ELSE li.reporting_curr_amt END AS reporting_curr_amt,
			CASE WHEN afe_op.afe_num IS NOT NULL
					AND vouchers.voucher_type_code IN ('ACCR','ACCRU','STACCR')
					AND li.grs_net_flag = 'NET'
					AND ISNULL(afe_op.net_estimate_pct, 0) <> 0
				THEN li.org_rep_curr_amt * 100 / ISNULL (afe_op.net_estimate_pct, 100)	/* Gross up only posted accruals for operated AFE's */
				ELSE li.org_rep_curr_amt END AS org_rep_curr_amt,
			CASE WHEN afe_op.afe_num IS NOT NULL
					AND vouchers.voucher_type_code IN ('ACCR','ACCRU','STACCR')
					AND li.grs_net_flag = 'NET'
					AND ISNULL (afe_op.net_estimate_pct, 0) <> 0
				THEN li.li_vol * 100 / ISNULL (afe_op.net_estimate_pct, 100)		/* Gross up only posted accruals for operated AFE's */
				ELSE li.li_vol END AS li_vol,
			CASE WHEN li.dest_org_id IS NULL THEN vouchers.org_id ELSE li.dest_org_id END AS org_id, 
			CASE WHEN vouchers.voucher_type_code IS NULL THEN '-1' 
				ELSE case when vouchers.voucher_stat_code = 'U' 
					then 'U_' + vouchers.voucher_type_code else vouchers.voucher_type_code end END AS acr_act_key,
		...
		, (		SELECT ae.afe_num,
				...
				INNER JOIN (	SELECT o.afe_num,
								...
								FROM (	SELECT DISTINCT FIRST_VALUE(client_id) 
														OVER (PARTITION BY afe_num ORDER BY last_update_date DESC, last_update_user, create_date) AS client_id,
												afe_num 
		...
		AND ((vouchers.voucher_type_code IN ('ACCR','ACCRU','STACCR')
			AND li.grs_net_flag    = 'NET') 
			OR (vouchers.voucher_type_code NOT IN ('ACCR','ACCRU','STACCR')
			AND li.grs_net_flag = 'CALC_GRS'))
		...
		AND li.afe_num IN (	SELECT DISTINCT ae.afe_num
							FROM (	SELECT DISTINCT l.afe_num
										, FIRST_VALUE(CASE maj.gross_or_net_code WHEN 'N' THEN 'NET' ELSE 'GRS' END)
											OVER (PARTITION BY l.afe_num 
												ORDER BY ISNULL(maj.gross_or_net_code,'A')) AS first_indicator_code
...
LEFT OUTER JOIN (	SELECT distinct account_id
