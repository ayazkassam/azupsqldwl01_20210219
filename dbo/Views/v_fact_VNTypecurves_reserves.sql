CREATE VIEW [dbo].[v_fact_VNTypecurves_reserves]
...
from (
	SELECT pf.entity_key,
		...
		gross_net_key,
		normalized_time_key,
		CASE WHEN e.reserve_realized_date IS NULL OR e.reserve_realized_date = '0' THEN '-1'
	...
	FROM [data_mart].[t_fact_valnav_reserves_typecurves] pf
	LEFT OUTER JOIN (
