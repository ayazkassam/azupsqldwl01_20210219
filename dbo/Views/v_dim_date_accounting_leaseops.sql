CREATE VIEW [dbo].[v_dim_date_accounting_leaseops] AS SELECT DISTINCT
...
SELECT [date_key]
      ...
      ,'Accounting ' + convert(char(8), [calendar_year_number]) as calendar_year_name
      ,[days_since_20000101]
  ...
  BETWEEN
      (SELECT int_value start_year
		...
		WHERE variable_name='LEASEOPS_ACCOUNTING_DATE_START_YEAR'
	   ) 
   AND
	(SELECT int_value end_year
	FROM [stage].t_ctrl_etl_variables
	WHERE variable_name='LEASEOPS_ACCOUNTING_DATE_END_YEAR')
  ) d1
 --
 UNION ALL
 --
...
(
 SELECT [date_key]
      ...
      ,[calendar_quarter_name_short]
     ,substring(LEFT(CONVERT(VARCHAR, [full_date], 120), 10),1,4) as calendar_year_number
      ,'Accounting ' + convert(char(8), calendar_month_name_short) as calendar_year_name
      ,[days_since_20000101]
      ,[weeks_since_20000101]
      ,[months_since_20000101]
      ,[quarters_since_20000101]
