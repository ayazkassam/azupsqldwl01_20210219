CREATE VIEW [dbo].[v_dim_date_accounting_capital]
AS SELECT DISTINCT
...
SELECT [date_key]
  ...
  BETWEEN
      (SELECT int_value start_year
		...
		WHERE variable_name='CAPITAL_ACCOUNTING_DATE_START_YEAR'
	   ) 
   AND
	(SELECT int_value end_year
	FROM [stage].[t_ctrl_etl_variables]
	WHERE variable_name='CAPITAL_ACCOUNTING_DATE_END_YEAR')
  ) d1
 --
 UNION ALL
 --
