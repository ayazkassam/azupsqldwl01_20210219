CREATE VIEW [dbo].[v_fact_leaseops_BCD] AS select li_id
	, voucher_id
	, entity_key
	, account_key
...
from (	
	...
	SELECT -1 li_id
		...
		, null cad_econ_variable_oil
		, null working_interest
	FROM [stage].[t_fact_source_leaseops_valnav_volumes] vv
	join [dbo].v_dim_scenario_leaseops_BCD slb
	on vv.scenario_key = slb.scenario_key
	---
	UNION ALL
	--
	/*---- OPEX ACCRUAL*/
