CREATE PROC [dbo].[usp_PopulateDimDate2] @piStartYear [INT],@piNumYears [INT] AS
BEGIN
   ...
   print cast(@cnt as varchar(10))+ ' before deletion'
   print convert(varchar(40), getdate(), 113)

  -- delete from [data_mart].[dim_date]

   print cast(@cnt as varchar(10))+ ' before loop'
   print convert(varchar(40), getdate(), 113)
   
   declare @start_loop_time datetime = getdate()
   declare @loop_time varchar(20)
   while (@liYear<@piStartYear+@piNumYears)
   begin
	  
	  set @start_loop_time = getdate()
	   
	   print cast(@cnt as varchar(10))+ ' start of the loop'
	   print convert(varchar(40), getdate(), 113)
      ...
      select @lsCalendarQuarterName=@lsCalendarYear+' '+@lsCalendarQuarterNameShort
      ...
      select @lsHolidayDesc='N/A'
	   ...
	   print cast(@cnt as varchar(10))+ ' before inserting'
	   print convert(varchar(40), getdate(), 113)
	  ...
	  set @cnt	= @cnt + 1
	  set @loop_time = cast(datediff(ms, @start_loop_time, getdate()) as varchar(20))
	   print cast(@cnt as varchar(10))+ ' end of the loop'
	   print convert(varchar(40), getdate(), 113)
	   print @loop_time + ' ms for one loop'
   end
END