CREATE VIEW [STAGE_METRIX].[v_fact_source_metrix_fees_incr]
AS select cast(convert(varchar(8), EOMONTH(cast(cast(d.production_date as varchar) + '01' as datetime)), 112) as int) AS activity_period
	...
	from [STAGE_METRIX].t_stg_metrix_fc_distributions_incr d
	join [STAGE_METRIX].t_stg_metrix_facility_charges_incr fc on d.facility_charge_sys_id = fc.sys_id and d.production_date = fc.production_date
