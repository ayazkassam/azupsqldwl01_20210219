CREATE PROC [data_mart].[incremental_ihs_fdc_facts] AS
BEGIN
...
BEGIN TRY	
    ...
    CREATE TABLE #t_fact_fdc_MERGE WITH (DISTRIBUTION = ROUND_ROBIN)
      AS	
        ...
        FROM  [stage].[v_fact_source_fdc_ihs_incr] as src
	...
	------ ROW_COUNT
	DECLARE  @rowcnt INT
	SELECT @rowcnt = COUNT(0) FROM #t_fact_fdc_MERGE WHERE Flag = 'NEW'
