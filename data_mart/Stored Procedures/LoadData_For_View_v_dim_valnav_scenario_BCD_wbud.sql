CREATE PROC [data_mart].[LoadData_For_View_v_dim_valnav_scenario_BCD_wbud] AS
BEGIN
	--IF OBJECT_ID('tempdb..#CTE') IS NOT NULL DROP TABLE #CTE;
    IF OBJECT_ID('[dbo].[CTE_v_dim_valnav_scenario_BCD_wbud]') IS NOT NULL 
	  TRUNCATE TABLE [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud]--DROP TABLE [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud];
    ELSE
	CREATE TABLE [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud]
	  (
	     [xlevel] INT,
	...
	INSERT INTO [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud]
	SELECT 1 AS xlevel,
	       scenario_key,
		   scenario_parent_key,
	...
	and scenario_key in ('Working Budget'	)
	...
	join [data_mart].t_dim_scenario child 
		on child.scenario_parent_key = [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud].scenario_key 
		and child.scenario_cube_name = [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud].scenario_cube_name
		WHERE [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud].xlevel = @counter
		)

	BEGIN

		-- Insert next level
		INSERT INTO [dbo].[CTE_v_dim_valnav_scenario_BCD_wbud] --( xlevel, FeatureId, ParentId, FeatureName, PathString, PathLength )
